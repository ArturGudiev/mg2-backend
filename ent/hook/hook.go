...
// The CardItemFunc type is an adapter to allow the use of ordinary
// function as CardItem mutator.
type CardItemFunc func(context.Context, *ent.CardItemMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f CardItemFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.CardItemMutation); ok {
		return f(ctx, mv)
	}
