...
type UserQuery struct {
	...
	withVerificationCodes *VerificationCodeQuery
...
// QueryMemoryNodes chains the current query on the "memory_nodes" edge.
func (_q *UserQuery) QueryMemoryNodes() *MemoryNodeQuery {
...
// QueryCardItems chains the current query on the "card_items" edge.
func (_q *UserQuery) QueryCardItems() *CardItemQuery {
	query := (&CardItemClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) QueryVerificationCodes() *VerificationCodeQuery {
	query := (&VerificationCodeClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithMemoryNodes tells the query-builder to eager-load the nodes that are connected to
// the "memory_nodes" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithMemoryNodes(opts ...func(*MemoryNodeQuery)) *UserQuery {
	query := (&MemoryNodeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withMemoryNodes = query
	return _q
}
...
// WithCardItems tells the query-builder to eager-load the nodes that are connected to
// the "card_items" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithCardItems(opts ...func(*CardItemQuery)) *UserQuery {
	query := (&CardItemClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) WithVerificationCodes(opts ...func(*VerificationCodeQuery)) *UserQuery {
	query := (&VerificationCodeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withMemoryNodes; query != nil {
		if err := _q.loadMemoryNodes(ctx, query, nodes,
			func(n *User) { n.Edges.MemoryNodes = []*MemoryNode{} },
			func(n *User, e *MemoryNode) { n.Edges.MemoryNodes = append(n.Edges.MemoryNodes, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withCardItems; query != nil {
	...
	if query := _q.withVerificationCodes; query != nil {
		if err := _q.loadVerificationCodes(ctx, query, nodes,
			func(n *User) { n.Edges.VerificationCodes = []*VerificationCode{} },
			func(n *User, e *VerificationCode) { n.Edges.VerificationCodes = append(n.Edges.VerificationCodes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadMemoryNodes(ctx context.Context, query *MemoryNodeQuery, nodes []*User, init func(*User), assign func(*User, *MemoryNode)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(memorynode.FieldUserID)
	}
	query.Where(predicate.MemoryNode(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.MemoryNodesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadCardItems(ctx context.Context, query *CardItemQuery, nodes []*User, init func(*User), assign func(*User, *CardItem)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(carditem.FieldUserID)
	}
...
func (_q *UserQuery) loadVerificationCodes(ctx context.Context, query *VerificationCodeQuery, nodes []*User, init func(*User), assign func(*User, *VerificationCode)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(verificationcode.FieldUserID)
	}
